...
export class CarsPage {
        ...
        this.apiServ.toGetCarInfo(this.user_session)
            .then(data => {
                ...
                if (this.response_get.status == "success") {
                    ...
                    this.error_session.text = "There was an error retrieving your car information, please try again later.";
        ...
        this.apiServ.toEnableAlarm(this.user_car_session)
            .then(data => {
                ...
                if (this.response_enable.status == "success") {
                    ...
                    this.wrong_session.text = "There was an error retrieving your car information, please try again later.";
    ...
    goToCarLocation() {
        if (this.user_car.alarm_active == false) {
            ...
            let modal = this.modal.create(MapPage, { 'lat': this.user_car.lat_last, 'long': this.user_car.long_last });
            modal.present();
        }
    }
    ...
    goToAskToDeleteCar() {
        let prompt = this.alert.create({
            title: 'Delete',
            message: "You are sure to remove this car from your account?",
            buttons: [
                {
                    text: 'Cancel',
                    handler: data => {
    ...
    goToDeleteCar() {
        this.apiServ.toDeleteCars(this.user_session)
            .then(data => {
                this.response_delete = data;
                if (this.response_delete.status == "success") {
                    let toast = this.toast.create({
                    ...
                    toast.present();
                } else if (this.response_delete.status == "error" && this.response_delete.type == "Empty car id") {
                    this.error_session.session_id = false;
                    this.error_session.session_token = false;
                    this.error_session.user = true;
                    this.error_session.text = "An error occurred the car I selected could not be foundplease try again later.";
                } else if (this.response_delete.status == "error" && this.response_delete.type == "session_id") {
                    this.error_session.session_id = true;
                    this.error_session.session_token = false;
                    this.error_session.user = false;
                    this.error_session.text = "There was an error retrieving your car information, please try again later.";
                } else if (this.response_delete.status == "error" && this.response_delete.type == "session_token") {
                    this.error_session.session_id = false;
                    this.error_session.session_token = true;
                    this.error_session.user = false;
                    this.error_session.text = "There was an error retrieving your car information, please try again later.";
                } else if (this.response_delete.status == "error" && this.response_delete.type == "Error on car") {
                    this.error_session.session_id = false;
                    this.error_session.session_token = false;
                    this.error_session.user = true;
                    this.error_session.text = "An error has occurred the car I select does not exist, please try again later.";
                } else if (this.response_delete.status == "error" && this.response_delete.type == "User not logged") {
                    this.error_session.session_id = true;
                    this.error_session.session_token = true;
                    this.error_session.user = false;
                    this.error_session.text = "An error occurred with the session, please try again later.";
                } else if (this.response_delete.status == "error" && this.response_delete.type == "Shared enabled") {
                    this.error_session.session_id = false;
                    this.error_session.session_token = false;
                    this.error_session.user = true;
                    this.error_session.text = "An error has occurred, you can not delete this vehicle while it is shared with another user.";
                }
            });
    }
    ...
    goToDashboard() {
        NativeStorage.remove('user_cars').then(() => { }, (error) => { });
        NativeStorage.remove('count_cars').then(() => { }, (error) => { });
        NativeStorage.setItem('user_cars', JSON.stringify(this.response_delete.user_cars)).then(() => { }, error => { });
        NativeStorage.setItem('count_cars', JSON.stringify(this.response_delete.count_cars)).then(() => { }, error => { });
